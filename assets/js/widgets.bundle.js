            ...
            if ("undefined" != typeof am5) {
                ...
                if (e) {
                    var t, a = function() {
                        ...
                        fetch(base_url + 'staff/monthly_spending')  // This is where your PHP endpoint is
                            .then(response => response.json())
                            .then(l => {
                                // l is now the dynamic data from PHP
                                ...
                                r.data.setAll(l); // Set the fetched data to the x-axis
                                ...
                                i.data.setAll(l); // Set the data dynamically
                                i.appear();
                                a.appear(1e3, 100);
                            })
                            .catch(error => console.error('Error fetching data:', error));  // Handle errors if any
                    };
...
var KTChartsWidget26 = function() {
    ...
    var t = function() {
        ...
        if (t) {
            ...
            fetch(base_url + 'staff/current_month_transaction', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({
                    action: 'getCurrentMonthData' // Example action, adjust as needed
                })
            })
            .then(response => response.json())
            .then(data => {
                var s = {
                    series: [{
                        name: t.getAttribute("data-kt-chart-info"),
                        data: data.chartData // The data returned from the server
                    }],
                    chart: {
                        fontFamily: "inherit",
                        type: "area",
                    ...
                    xaxis: {
                        categories: data.categories, // Categories from server response
                        axisBorder: {
                            show: !1
                        },
                        axisTicks: {
...
function getCurrentMonthCategories() {
    const today = new Date();
    const year = today.getFullYear();
    const month = today.getMonth(); // 0-based index for months (0 = January, 11 = December)
    const currentDay = today.getDate(); // Today's day of the month

    const categories = [""]; // Start with an empty string if needed
    for (let day = 1; day <= currentDay; day++) {
        const date = new Date(year, month, day);
        const formattedDate = date.toLocaleDateString("en-US", { month: "short", day: "2-digit" });
        categories.push(formattedDate); // Format as "Apr 02"
    }
    return categories;
}
